...
- Scipy/NumPy: Core interpolation remains scipy (interp1d/splrep/splev); no numpy-only
replacement for splines. Index filtering is vectorized with numpy.

- JIT: The main path cannot be JIT-compiled (scipy is not numba-compatible). An optional
numba fast path for degree==1, xCoords is None, smooth==0 could accelerate the
uniform linear case by filling NaN runs in one pass without building an interp1d.
"""
...
def interpolateNonFinite(   signal : np.array, 
    ...
    if xCoords is not None:
        ...
        # xCoords = np.array(range(lSignal)) / lSignal # original code
        xCoords = np.linspace(0, 1, lSignal, endpoint=False) # revised code
    ...
    # interpolateAtIndexes = np.setxor1d(range(lSignal), finiteIndexes) # original code
    interpolateAtIndexes = np.setxor1d(np.arange(lSignal), finiteIndexes) # revised code
