...
This module contains the functions for the remezord function from eegpy package.

Functions:
    remezord: Filter parameter selection for the Remez exchange algorithm.
        Args:
            freqs (numpy.ndarray): A monotonic sequence of band edges specified in Hertz. All elements must be non-negative and less than 1/2 the sampling frequency as given by the Hz parameter.
            amps (numpy.ndarray): A sequence containing the amplitudes of the signal to be filtered over the various bands.
            rips (numpy.ndarray): A sequence specifying the maximum ripples of each band.
            Hz (float): Sampling frequency in Hertz.
            alg (str): Filter length approximation algorithm. May be 'herrmann', 'kaiser', or 'ichige'.

        Returns:
            tuple: (int, numpy.ndarray, numpy.ndarray, numpy.ndarray)
                The filter length, bands, desired, and weight.

        Raises:
            ValueError: If the frequency band edges exceed the Nyquist frequency.
            ValueError: If the frequency band edges are negative.
            ValueError: If the ripples are negative.
            ValueError: If the number of amplitudes does not equal the number of ripples.
            ValueError: If the number of band edges does not equal 2*((number of amplitudes)-1).
            ValueError: If the filter length approximation algorithm is unknown.
"""

import numpy
from numpy import asarray, pi, absolute, sqrt, log10, arctan, ceil, hstack, mod
...
def findfreqs(num, den, N):
    m = numpy.arange(0, N)
    h = win * special.sinc(cutoff * (m - alpha))  # noqa: F821
...
def remlplen_herrmann(fp, fs, dp, ds):
    ...
    b = [11.01217, 0.51244]
    Dinf = (
        log10(ds) * (a[0] * log10(dp) ** 2 + a[1] * log10(dp) + a[2])
        + a[3] * log10(dp) ** 2
        + a[4] * log10(dp)
        + a[5]
    )
...
def remlplen_ichige(fp, fs, dp, ds):
    ...
    dF = fs - fp
    v = lambda dF, dp: 2.325 * ((-log10(dp)) ** -0.445) * dF ** (-1.39)  # noqa: E731
    g = lambda fp, dF, d: (2.0 / pi) * arctan(v(dF, dp) * (1.0 / fp - 1.0 / (0.5 - dF)))  # noqa: E731
    h = lambda fp, dF, c: (2.0 / pi) * arctan((c / dF) * (1.0 / fp - 1.0 / (0.5 - dF)))  # noqa: E731
