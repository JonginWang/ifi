...
@contextmanager
def interactive_plotting(show_plots: bool = True, save_dir: Optional[str] = None, 
                        save_prefix: str = "fig_", save_ext: str = "png", 
                        dpi: int = 300, block: bool = True):
    """
...
class Plotter:
    ...
    def _prepare_time_data(self, data: Union[pd.DataFrame, Dict[str, np.ndarray], np.ndarray], 
                          fs: Optional[float] = None) -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
        """
        ...
        if isinstance(data, pd.DataFrame):
            ...
            if data.ndim == 1 and fs is not None:
                ...
                logging.error(f"Waveform data must be 1D or 2D numpy array. Got {data.shape}")
                raise ValueError("Waveform data must be 1D or 2D numpy array")
        else:
            logging.error(f"Waveform data must be DataFrame, dict, or numpy array. Got {type(data)}")
            raise ValueError("Waveform data must be DataFrame, dict, or numpy array")
            
        return time, signals
    
    def _apply_scaling(self, time: np.ndarray, signals: Dict[str, np.ndarray], 
                      time_scale: str = 's', signal_scale: str = 'V') -> Tuple[np.ndarray, Dict[str, np.ndarray], str, str]:
        """
            ...
            if signal_scale == 'mV':
                ...
                if '^-' in signal_scale:
                    def replace_superscript(match):
                        ...
                        exp = match.group(2)
                        return f'{base}^{{-{exp}}}'
                    signal_label = r'[${' + re.sub(r'(\w+)\^-(\d+)', replace_superscript, signal_scale) + r'}$]'
                else:
    ...
    def _extract_metadata_info(self, data: Union[pd.DataFrame, Dict[str, np.ndarray], np.ndarray]) -> str:
        """
    ...
    def plot_waveforms(self, data: Union[pd.DataFrame, Dict[str, np.ndarray], np.ndarray], 
                      ...
                      time_scale: str = 's',
                      signal_scale: str = 'V',
                      trigger_time: float = 0.0) -> Tuple[Figure, np.ndarray]:
        """
    ...
    def plot_time_frequency(self, data: Union[pd.DataFrame, Dict[str, np.ndarray], np.ndarray, Tuple[np.ndarray, np.ndarray, np.ndarray]],
                           method: str = 'stft',
                           fs: Optional[float] = None,
        ...
        for i, (name, signal) in enumerate(signals.items()):
            ...
            if method.lower() == 'precomputed':
                ...
                # Plot spectrogram
                im = ax.pcolormesh(times_scaled, freqs_scaled, Sxx_plot, 
                ...
                # Plot spectrogram
                im = ax.pcolormesh(times * time_scale_factor, freqs_scaled, Sxx_plot, 
                                 shading='gouraud', cmap='viridis')
                
                # Add frequency ridge if available
                try:
                    ridge = self.analyzer.find_freq_ridge(Sxx, freqs, method='stft')
                    ax.plot(times * time_scale_factor, ridge * freq_scale_factor, 
                           color='r', linewidth=2, label='Frequency Ridge')
                    ax.legend()
                except Exception:
                    pass
                ...
                # Plot scalogram
                im = ax.pcolormesh(time * time_scale_factor, freqs_scaled, cwt_matrix, 
                                 shading='gouraud', cmap='hot')
                power_label = "Magnitude"
            
            else:
                raise ValueError(f"Unknown method: {method}. Use 'stft', 'cwt', or 'precomputed'")
    ...
    def plot_density(self, density_data: Union[pd.DataFrame, Dict[str, np.ndarray], np.ndarray], 
                    time_data: Optional[np.ndarray] = None, 
                    title: str = "Density Results", 
                    save_path: Optional[str] = None,
                    show_plot: bool = True,
                    density_scale: str = '10^18 m^-3',
                    time_scale: str = 's') -> Tuple[Figure, np.ndarray]:
        """
        ...
        time_scaled, _, time_label, _ = self._apply_scaling(time_data, {"dummy": np.zeros_like(time_data)}, time_scale)
        _, density_scaled, _, density_label = self._apply_scaling(time_data, signals, time_scale, density_scale)
    ...
    def plot_response(self, freqs: np.ndarray, responses: np.ndarray, 
                     title: str = "Frequency Response",
                     save_path: Optional[str] = None,
                     show_plot: bool = True,
                     freq_scale: str = 'Hz') -> Tuple[Figure, np.ndarray]:
        """
    ...
    def plot_shot_overview(self, shot_data: dict, vest_data: pd.DataFrame, 
                          results_dir: Path, shot_num: int,
                          save_path: Optional[str] = None,
...
def plot_shot_waveforms(shot_data: dict, results_dir: Path, shot_num: int, downsample: int = 100):
...
def plot_shot_spectrograms(shot_data: dict, results_dir: Path, shot_num: int, max_channels: int = 2):
...
def plot_shot_density_evolution(shot_data: dict, vest_data: pd.DataFrame, results_dir: Path, shot_num: int):
...
def plot_shot_overview(shot_data, vest_data, results_dir, shot_num, **kwargs):
    """Legacy function - now uses Plotter class."""
    plotter = Plotter()
    return plotter.plot_shot_overview(shot_data, vest_data, results_dir, shot_num, **kwargs)
...
if __name__ == '__main__':
    ...
    try:
        ...
        fig1, axes1 = plotter.plot_waveforms(data, title="Test Signals", 
                                           time_scale='ms', signal_scale='mV', 
                                           show_plot=False, save_path=demo_dir / "waveforms_mV.png")
        
        # Example 2: Time-frequency analysis
        print("2. Time-frequency analysis (STFT)...")
        signal = np.sin(2 * np.pi * 10 * t) + 0.5 * np.sin(2 * np.pi * 20 * t)
        fig2, axes2 = plotter.plot_time_frequency(signal, method='stft', fs=1000, 
                                                title="Test STFT", show_plot=False,
        ...
        Sxx = np.random.rand(100, 200) * np.exp(-((freqs[:, None] - 25)**2 + (times[None, :] - 0.5)**2) / 0.1)
        fig3, axes3 = plotter.plot_time_frequency((freqs, times, Sxx), method='precomputed', 
                                                title="Pre-computed STFT", show_plot=False,
        ...
        fig4, ax4 = plotter.plot_density(density_data, density_scale='10^18 m^-3', 
                                       title="Density Evolution", show_plot=False,
                                       save_path=demo_dir / "density_evolution.png")
        
        # Example 5: LaTeX superscript formatting
        print("5. LaTeX superscript formatting...")
        fig5, ax5 = plotter.plot_waveforms(data, signal_scale='m^-2', 
                                         title="Test with LaTeX superscript", show_plot=False,
