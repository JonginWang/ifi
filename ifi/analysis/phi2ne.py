...
class PhaseConverter:
    ...
    def calc_phase_cdm(self, ref_signal: np.ndarray, prob_signal: np.ndarray, fs: float, f_center: float, isbpf: bool = True, isconj: bool = False, isold: bool = False, isflip: bool = False, plot_filters: bool = False) -> np.ndarray:
        ...
        if f_center > 60e6:
            fcl, f0l, f0r, fcr = f_center - 12.5e6, f_center - 7.5e6, f_center + 7.5e6, f_center + 12.5e6
        elif f_center > 30e6:
            fcl, f0l, f0r, fcr = f_center - 10.0e6, f_center - 5.0e6, f_center + 5.0e6, f_center + 10.0e6
        else:
            fcl, f0l, f0r, fcr = f_center -  7.0e6, f_center - 3.0e6, f_center + 3.0e6, f_center +  7.0e6
    ...
    def correct_baseline(self, density_df: pd.DataFrame, time_axis: np.ndarray, mode: str, shot_num: int = None, vest_data: pd.DataFrame = None, ip_column_name: str = None):
        """
        ...
        if mode == 'ip':
            if shot_num is None:
                logging.warning("    ! [Baseline Correction] 'ip' baseline mode selected but shot_num not provided. Skipping.")
                return corrected_df
