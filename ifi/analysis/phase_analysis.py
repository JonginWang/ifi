...
@njit
def _cordic_rotation_vectorized_jit(x: np.ndarray, y: np.ndarray, target_angles: np.ndarray, 
                                   angles: np.ndarray, n_iterations: int, scale_factor: float) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    for i in range(n_iterations):
        ...
        for j in range(n):
            ...
            should_rotate = directions[j] * angle_accum[j] < directions[j] * target_angles[j]
...
class CORDICProcessor:
    ...
    def __init__(self, n_iterations: int = 16):
        ...
        self.scale_factor = np.prod(np.sqrt(1.0 + 2.0**(-2*np.arange(n_iterations))))
    
    def cordic_rotation(self, x: float, y: float, target_angle: float) -> Tuple[float, float, float]:
        """
    ...
    def cordic_rotation_vectorized(self, x: np.ndarray, y: np.ndarray, target_angles: np.ndarray) -> Tuple[np.ndarray, np.ndarray, float]:
        """
    ...
    def _cordic_rotation_numpy_vectorized(self, x: np.ndarray, y: np.ndarray, target_angles: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        for i in range(self.n_iterations):
            ...
            # Apply rotations where needed
            x_new = np.where(should_rotate,
                           x_rot - directions * y_rot * factor,
                           x_rot + directions * y_rot * factor)
            y_new = np.where(should_rotate,
                           y_rot + directions * x_rot * factor,
                           y_rot - directions * x_rot * factor)
            angle_accum = np.where(should_rotate,
    ...
    def extract_phase_samples(self, signal: np.ndarray, f0: float, fs: float, 
                            samples_per_period: int = 4) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        magnitudes, phases, _ = self.cordic_rotation_vectorized(x_values, y_values, np.zeros(len(x_values)))
...
class SignalStacker:
    ...
    def find_fundamental_frequency(self, signal: np.ndarray, f_range: Tuple[float, float] = (0.1, None)) -> float:
        """
        ...
        if f_range[1] is None:
            ...
            f_range = (validate_frequency(f_range[0], "f_range[0]"), 
    ...
    def stack_signals(self, signal: np.ndarray, f0: float, n_stacks: int = 4, 
                     time_axis: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        """
        # Validate input parameters
        from ifi.utils.validation import validate_signal, validate_frequency, validate_positive_number
        signal = validate_signal(signal, "signal", min_length=2)
        ...
        if time_axis is not None:
            ...
            for n in range(int(n_stacks)):
                ...
                for i in range(0, len(signal) - int(interval_samples) + 1, int(interval_samples)):
                ...
                if segments:
                    ...
                    for i, seg in enumerate(segments):
                        ...
                        if end_idx <= len(stacked_signal):
                            stacked_signal[start_idx:end_idx] += stacked_segment * weight
                            total_weight += weight
    ...
    def compute_phase_difference_cdm(self, ref_signal: np.ndarray, probe_signal: np.ndarray, 
                                   f0: Optional[float] = None) -> Tuple[np.ndarray, float]:
        """
        ...
        if f0 >= self.fs / 2:
            raise ValueError(f"Fundamental frequency {f0:.2f} Hz exceeds Nyquist frequency {self.fs/2:.2f} Hz. "
        ...
        if f0 >= 0.9 * self.fs / 2:
            import logging
            logging.warning(f"Fundamental frequency {f0:.2f} Hz is close to Nyquist frequency {self.fs/2:.2f} Hz. "
        ...
        try:
            ...
            raise RuntimeError(f"CDM method failed: {e}. This may indicate insufficient sampling rate "
                             f"or invalid signal characteristics for CDM analysis.") from e
        
        return phase_diff, f0
    
    def compute_phase_difference_cordic(self, ref_signal: np.ndarray, probe_signal: np.ndarray, 
                                      f0: Optional[float] = None) -> Tuple[np.ndarray, np.ndarray, float]:
        """
        ...
        ref_times, ref_phases = self.cordic_processor.extract_phase_samples(ref_signal, f0, self.fs)
        probe_times, probe_phases = self.cordic_processor.extract_phase_samples(probe_signal, f0, self.fs)
    ...
    def detect_phase_changes_unified(self, ref_signal: np.ndarray, probe_signal: np.ndarray, 
                                   f0: Optional[float] = None, method: str = 'stacking') -> Dict[str, Any]:
        """
        ...
        if method == 'stacking':
            phase_diff, detected_f0 = self.compute_phase_difference(ref_signal, probe_signal, f0, 'stacking')
            times = np.arange(len(phase_diff)) / self.fs
        elif method == 'cdm':
            phase_diff, detected_f0 = self.compute_phase_difference(ref_signal, probe_signal, f0, 'cdm')
            times = np.arange(len(phase_diff)) / self.fs
        elif method == 'cordic':
            times, phase_diff, detected_f0 = self.compute_phase_difference_cordic(ref_signal, probe_signal, f0)
        else:
    ...
    def _analyze_phase_changes_comprehensive(self, phase_diff: np.ndarray, times: np.ndarray, 
        ...
        if len(phase_diff) > 4:
            ...
            y_cubic = cubic_coeffs[0] * x**3 + cubic_coeffs[1] * x**2 + cubic_coeffs[2] * x + cubic_coeffs[3]
        ...
        change_type = self._classify_phase_change_type(
            r_squared_linear, r_squared_quad, r_squared_cubic, 
            constant_std, linear_slope, quad_coeffs[0] if len(quad_coeffs) > 0 else 0
        )
        ...
        return {
            ...
            'model_comparison': {
                'linear_vs_constant': r_squared_linear - 0.5,  # Threshold for linear detection
                'quadratic_vs_linear': r_squared_quad - r_squared_linear,
                'cubic_vs_quadratic': r_squared_cubic - r_squared_quad
            }
        }
    
    def _classify_phase_change_type(self, r_linear: float, r_quad: float, r_cubic: float,
                                   constant_std: float, linear_slope: float, quad_coeff: float) -> str:
        """
    ...
    def _estimate_signal_quality(self, phase_diff: np.ndarray, residuals: np.ndarray) -> Dict[str, float]:
        """
        ...
        linearity_index = 1.0 - (np.var(residuals) / np.var(phase_diff)) if np.var(phase_diff) > 0 else 0
    ...
    def analyze_linear_phase_drift(self, ref_signal: np.ndarray, probe_signal: np.ndarray, 
                                  f0: Optional[float] = None) -> Dict[str, Any]:
        """
    ...
    def compute_phase_difference(self, ref_signal: np.ndarray, probe_signal: np.ndarray, 
                               f0: Optional[float] = None, method: str = 'cdm') -> Tuple[np.ndarray, float]:
        """
        ...
        """
        # Validate input parameters
        from ifi.utils.validation import validate_signal, validate_signals_match, validate_method
        ...
        if method == 'cdm':
            ...
            times, phase_diff, f0 = self.compute_phase_difference_cordic(ref_signal, probe_signal, f0)
            return phase_diff, f0
            ...
            raise ValueError(f"Unknown method: {method}. Use 'cdm', 'cordic', or 'stacking'")
...
class STFTRidgeAnalyzer:
    ...
    def compute_stft_with_ridge(self, signal: np.ndarray, f0: float, 
    ...
    def extract_phase_from_ridge(self, signal: np.ndarray, ridge_freqs: np.ndarray, 
                               times: np.ndarray, nperseg: int = 1024, noverlap: int = 512) -> np.ndarray:
        """
    ...
    def compute_phase_difference(self, ref_signal: np.ndarray, probe_signal: np.ndarray, 
                               f0: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        ...
        f_probe, t_probe, ridge_freqs_probe = self.compute_stft_with_ridge(probe_signal, f0)
        ...
        # Extract phases
        ref_phases = self.extract_phase_from_ridge(ref_signal, ridge_freqs_common, t_common)
        probe_phases = self.extract_phase_from_ridge(probe_signal, ridge_freqs_common, t_common)
...
class CWTPhaseReconstructor:
    ...
    def design_fir_filter(self, f0: float, bandwidth: float = 0.1) -> np.ndarray:
        ...
        # Design FIR filter
        filter_order = 101
        filter_coeffs = spsig.firwin(filter_order, [low_norm, high_norm], 
                                   pass_zero=False, window='hamming')
        
        return filter_coeffs
    
    def decimate_signal(self, signal: np.ndarray, decimation_factor: int = 4) -> np.ndarray:
        """
    ...
    def compute_cwt_phase(self, signal: np.ndarray, f0: float, 
                         voices_per_octave: int = 16) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def compute_phase_difference(self, ref_signal: np.ndarray, probe_signal: np.ndarray, 
                               f0: float, decimation_factor: int = 4) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
...
class PhaseChangeDetector:
    ...
    def detect_phase_changes(self, ref_signal: np.ndarray, probe_signal: np.ndarray, 
                           f0: Optional[float] = None, methods: List[str] = None) -> Dict[str, Any]:
        """
        ...
        if 'cordic' in methods:
            try:
                times, phase_diff, detected_f0 = self.signal_stacker.compute_phase_difference_cordic(
        ...
        if 'stft' in methods:
            try:
                times, ref_phases, probe_phases = self.stft_analyzer.compute_phase_difference(
                    ref_signal, probe_signal, f0)
        ...
        if 'cwt' in methods:
            try:
                times, ref_phases, probe_phases = self.cwt_reconstructor.compute_phase_difference(
                    ref_signal, probe_signal, f0)
                phase_diff = probe_phases - ref_phases
                results['methods']['cwt'] = {
                    'times': times,
    ...
    def compare_methods(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for method_name, method_result in methods.items():
            if 'error' in method_result:
                comparison['method_comparison'][method_name] = {'status': 'failed', 'error': method_result['error']}
                continue
        ...
        successful_methods = [name for name, result in comparison['method_comparison'].items() 
                            if result['status'] == 'success']
        
        if len(successful_methods) > 1:
            # Compare consistency
            phase_means = [comparison['method_comparison'][name]['mean_phase_diff'] 
                          for name in successful_methods]
            phase_stds = [comparison['method_comparison'][name]['std_phase_diff'] 
                         for name in successful_methods]
            ...
            if mean_consistency < 0.1:  # Less than 0.1 radian difference
                comparison['recommendations'].append("Methods show good consistency in phase detection")
            else:
                comparison['recommendations'].append("Methods show significant differences - check signal quality")
        ...
        if successful_methods:
            ...
            comparison['recommendations'].append(f"Recommended method: {best_method} (lowest phase variance)")
    ...
    def analyze_linear_phase_drift(self, ref_signal: np.ndarray, probe_signal: np.ndarray, 
        ...
        if 'stacking' in methods:
            try:
                drift_analysis = self.signal_stacker.analyze_linear_phase_drift(ref_signal, probe_signal, f0)
        ...
        if 'stft' in methods:
            try:
                times, ref_phases, probe_phases = self.stft_analyzer.compute_phase_difference(ref_signal, probe_signal, f0)
                phase_diff = probe_phases - ref_phases
        ...
        if 'cwt' in methods:
            try:
                times, ref_phases, probe_phases = self.cwt_reconstructor.compute_phase_difference(ref_signal, probe_signal, f0)
                phase_diff = probe_phases - ref_phases
        ...
        if 'cdm' in methods:
            try:
                phase_diff, detected_f0 = self.signal_stacker.compute_phase_difference(ref_signal, probe_signal, f0, method='cdm')
