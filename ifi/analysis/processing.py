...
def refine_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not isinstance(df, pd.DataFrame):
        logger.warning("refine_data called with non-DataFrame input. Skipping.")
        return df
    ...
    logger.info("    - [Refine Data] Rounded numeric data to 10 decimal places.")
    ...
    if "TIME" in refined_df.columns and len(refined_df["TIME"]) > 1:
        ...
        if not time_diffs.empty:
            ...
            logger.info(
                f"    - [Refine Data] Reconstructed 'TIME' column with resolution: {time_resolution:.4e} s"
            )
        else:
            logger.warning(
                "    - [Refine Data] Could not determine time resolution. Skipping TIME reconstruction."
            )
    ...
    if initial_rows > final_rows:
        logger.info(
            f"    - [Refine Data] Dropped {initial_rows - final_rows} rows with NaN values."
        )
...
def remove_offset(df: pd.DataFrame, window_size: int = 2001) -> pd.DataFrame:
    ...
    if not isinstance(df, pd.DataFrame):
        logger.warning(
            "    ! [Remove Offset] remove_offset called with non-DataFrame input. Skipping."
        )
        return df

    # Skip offset removal for FPGA data (.dat files)
    if df.attrs.get("source_file_type") == "dat":
        logger.info("    - [Remove Offset] Skipping offset removal for FPGA data file.")
        return df.copy()

    # If window_size is negative, use default.
    if window_size < 0:
        logger.warning(
            f"    ! [Remove Offset] Negative window_size ({window_size}) is invalid. Using default 2001."
        )
        window_size = 2001
    ...
    if 0 < num_rows < 1_000_000:
        ...
        if dynamic_window_size < window_size:
            ...
            logger.info(
                f"    - [Remove Offset] Dynamically adjusted window size to {window_size} for smaller dataset ({num_rows} rows)."
            )

    if window_size <= 0:
        logger.info(
            "    - [Remove Offset] Offset removal skipped as window_size is zero or negative."
        )
        return df.copy()
    ...
    if window_size % 2 == 0:
        window_size += 1
        logger.warning(
            f"    ! [Remove Offset] Window size must be odd. Adjusting to {window_size}."
        )
    ...
    logger.info(
        f"    - [Remove Offset] Removed offset using a {window_size}-point moving average. ({rows_removed} rows removed at edges)"
    )
