#!/usr/bin/env python3
"""
Comprehensive test suite for plotting functions with real and synthetic data.
"""
...
def test_spectrum_plots(logger):
    ...
    try:
        ...
        output_path = os.path.join(output_dir, "cwt_spectrogram.png")
        fig.savefig(output_path, dpi=150, bbox_inches='tight')
        plt.close(fig)
...
def test_density_plots(logger):
    ...
    try:
        ...
        output_path = os.path.join(output_dir, "density_evolution.png")
        fig.savefig(output_path, dpi=150, bbox_inches='tight')
        plt.close(fig)
    ...
    for channel, ne_data in density_profiles.items():
        try:
            ...
            ax1.legend()
            ax1.set_title(f'Plasma Density - {channel}')
            ...
            ax2.set_ylabel('Ip (V)', color='r')
            ax2_twin.set_ylabel('H-alpha (a.u.)', color='g')
            ax2.grid(True, alpha=0.3)
            
            plt.tight_layout()
            
            output_path = os.path.join(output_dir, f"{channel}_individual.png")
            fig.savefig(output_path, dpi=150, bbox_inches='tight')
            plt.close(fig)
...
def test_comparison_plots(logger):
    ...
    try:
        ...
        axes[1, 1].set_title('CWT Spectrogram')
...
def performance_test(logger):
    ...
    for size in data_sizes:
        ...
        try:
            ...
            ax.set_title(f'Performance Test - {size:,} samples')
            plt.close(fig)
