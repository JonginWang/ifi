#!/usr/bin/env python3
"""
Full integration test suite combining analysis, spectrum, and plotting functions.
"""
    ...
    for filename, file_data in results.items():
        ...
        try:
            if params['ref_col']:
                ...
                ridge_stft = analyzer.find_freq_ridge(Sxx, f_stft)
                ...
                if len(ref_signal) <= 1000000:  # Only for manageable sizes
                    ...
                    ridge_cwt = analyzer.find_freq_ridge(Wx, freqs_cwt)
                    logger.info(f"   CWT ridge: {np.mean(ridge_cwt)/1e6:.2f} MHz")
                else:
                    logger.info(f"   CWT skipped (signal too long)")
                ...
                logger.info(f"     No reference signal for analysis")
