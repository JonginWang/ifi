...
class VestDbPollingWorker(threading.Thread):
    ...
    Args:
        vest_db: Connected VestDB instance to query.
    ...
    def run(self) -> None:
        ...
        logger.info(f"{log_tag('VESTD', 'POLL')} VEST DB polling worker started.")
            ...
            try:
                ...
                if isinstance(shot_code, int):
                    if self._last_shot_code is None:
                        ...
                        logger.info(
                            f"{log_tag('VESTD', 'POLL')} New shot code detected: {shot_code}"
                        )
                        self._last_shot_code = shot_code
                ...
                logger.error(
                    f"{log_tag('VESTD', 'POLL')} Error while polling VEST DB: {exc}"
                )

            time.sleep(self._poll_interval)

        logger.info(f"{log_tag('VESTD', 'POLL')} VEST DB polling worker stopped.")
