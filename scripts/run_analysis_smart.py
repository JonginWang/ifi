...
def group_shots_for_execution(file_counts: dict[int, int]) -> List[Tuple[List[int], str]]:
    ...
    Strategy:
    - run exactly one shot per analysis process
    ...
    for shot, count in sorted(file_counts.items()):
        if count > 0:
            execution_groups.append(([shot], f"{count} file(s)"))
...
Examples:
  ...
  python scripts/run_analysis_smart.py --freq 280 --density --stft --save_data
    ...
    nas_db = NasDB(config_path=str(config_path))
    try:
