...
def sample_dataframe_with_korean():
    ...
    df = pd.DataFrame({
        "TIME": t,
        "?쒓?而щ읆": np.sin(2 * np.pi * 8e6 * t),  # Korean column name
...
class TestH5StructureValidation:
    ...
    Tests based on:
    - ifi/utils/io_utils.py: save_results_to_hdf5, load_results_from_hdf5
    ...
    def test_h5_save_and_load_structure(self, tmp_h5_dir, sample_dataframe):
        ...
        saved_h5_path = io_utils.save_results_to_hdf5(
            str(tmp_h5_dir),
            shot_num,
            signals,
            stft_results,
        ...
        with h5py.File(saved_h5_path, "r") as hf:
            assert hf.attrs["shot_number"] == shot_num
            assert "created_at" in hf.attrs
            assert "ifi_version" in hf.attrs
            
            assert "rawdata" in hf, "Rawdata group should exist"
            signal_group = next(
                group
                for group in hf["rawdata"].values()
                if isinstance(group, h5py.Group)
                and group.attrs.get("original_name") == "test_file.csv"
            )
            ...
            assert "density" in hf, "Density group should exist"
            assert "freq_94G" in hf["density"]
            assert "ne_CH0_test" in hf["density"]["freq_94G"]
            assert "ne_CH1_test" in hf["density"]["freq_94G"]
            
            assert "vestdata" in hf, "VEST data group should exist"
    ...
    def test_h5_column_names_validation(self, tmp_h5_dir, sample_dataframe):
        ...
        h5_path = io_utils.save_results_to_hdf5(
            str(tmp_h5_dir),
            shot_num,
            signals,
            {},
    ...
    def test_h5_metadata_preservation(self, tmp_h5_dir, sample_dataframe):
        ...
        loaded_results = io_utils.load_results_from_hdf5(shot_num, base_dir=base_dir)
    ...
    def test_h5_empty_signals_handling(self, tmp_h5_dir):
        ...
        with h5py.File(h5_path, "r") as hf:
            assert "rawdata" in hf
            assert hf["rawdata"].attrs.get("empty", False)
    
    def test_h5_format_consistency_with_nas_db_cache(self, tmp_h5_dir, sample_dataframe):
        """Test that H5 format from save_results_to_hdf5 is consistent with NasDB cache format.
        
        NasDB uses pandas.to_hdf() with format='table' for caching.
        save_results_to_hdf5 uses h5py for structured storage.
        This test validates compatibility between the two formats.
        """
        shot_num = 45821
        
        # Save using io_utils.save_results_to_hdf5
        signals = {"test_file.csv": sample_dataframe}
        h5_path = io_utils.save_results_to_hdf5(
            str(tmp_h5_dir),
            shot_num,
    ...
    def test_h5_load_compatibility(self, tmp_h5_dir, sample_dataframe):
        ...
        assert loaded_results is not None
        assert "metadata" in loaded_results
        assert "rawdata" in loaded_results
        assert "test_file.csv" in loaded_results["rawdata"]
        
        # Verify signal data integrity
        loaded_df = loaded_results["rawdata"]["test_file.csv"]
        assert len(loaded_df) == len(sample_dataframe)
        assert "TIME" in loaded_df.columns
        assert "CH0" in loaded_df.columns
        
        # Verify STFT results
        assert "stft" in loaded_results
        assert "test_file.csv" in loaded_results["stft"]
        
        # Verify density data
        assert "density" in loaded_results
        assert "freq_94G" in loaded_results["density"]
        assert "ne_CH0_test" in loaded_results["density"]["freq_94G"].columns
        
        # Verify VEST data
        assert "vestdata" in loaded_results
        assert "25k" in loaded_results["vestdata"]
        assert "ip" in loaded_results["vestdata"]["25k"].columns
    ...
    def test_h5_post_processing_data_format(self, tmp_h5_dir, sample_dataframe):
        ...
        - root attrs: shot number, creation time, IFI version
        - rawdata/: Raw signal data (DataFrames as groups with column datasets)
        - stft/: STFT analysis results (nested groups per signal)
        - cwt/: CWT analysis results (nested groups per signal)
        - density/: Density calculation results (column datasets)
        - vestdata/: VEST database data grouped by sample rate
        """
        shot_num = 45821
        ...
        with h5py.File(h5_path, "r") as hf:
            ...
            assert "density" in hf
            assert "ne_CH0_combined" in hf["density"]
            assert "ne_CH1_combined" in hf["density"]
            ...
            assert len(hf["density"]["ne_CH0_combined"]) > 0, "Density data should not be empty"
            assert len(signal_group["TIME"]) > 0, "Signal data should not be empty"
...
class TestPlotValidation:
    ...
    def test_plot_title_sanitization(self):
        ...
        test_title = "Shot #45821 - Korean title test"
...
class TestDaskParallelProcessing:
    ...
    def test_dask_parallel_post_processing(
        ...
        4. Each task creates its own NasDB instance
        ...
        # Mock NasDB instance
        mock_nas_instance = Mock(spec=NasDB)
        ...
        # Verify that NasDB was instantiated for each task
        assert mock_nas_class.call_count == len(test_files), "Each task should create its own NasDB instance"
...
class TestMainAnalysisIntegration:
    ...
    @patch("ifi.analysis.main_analysis.NasDB")
    @patch("ifi.analysis.main_analysis.VestDB")
    def test_full_analysis_workflow(self, mock_vest_class, mock_nas_class, 
        ...
        mock_nas_db = Mock(spec=NasDB)
        mock_vest_db = Mock(spec=VestDB)
        ...
        with h5py.File(h5_file, "r") as hf:
            ...
            # Check metadata
            if "shot_number" in hf.attrs:
                print(f"Metadata attrs: {dict(hf.attrs)}")
                assert hf.attrs["shot_number"] == 45821, "Shot number should be 45821"
            else:
                print("WARNING: root metadata attrs not found")
            
            # Check rawdata
            if "rawdata" in hf:
                print(f"Rawdata groups: {list(hf['rawdata'].keys())}")
                if hf["rawdata"].attrs.get("empty", False):
                    print("WARNING: rawdata group is empty")
            else:
                print("WARNING: rawdata group not found")
            
            # Check density
            if "density" in hf:
                print(f"Density groups: {list(hf['density'].keys())}")
            else:
                print("WARNING: density group not found")
            
            # Check stft
            if "stft" in hf:
                print(f"STFT groups: {list(hf['stft'].keys())}")
            else:
                print("INFO: stft group not found (may be empty)")
