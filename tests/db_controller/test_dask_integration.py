...
def test_db_controller_integration():
    ...
    try:
        ...
        if nas_db.connect():
            ...
            print("  ?ая╕П NAS_DB connection failed (may be expected in test environment)")
        ...
        if vest_db.connect():
            ...
            print("  ?ая╕П VEST_DB connection failed (may be expected in test environment)")
            
    except FileNotFoundError:
        print("  ?ая╕П Configuration file not found (expected in test environment)")
    except Exception as e:
        print(f"  ??Error: {e}")
