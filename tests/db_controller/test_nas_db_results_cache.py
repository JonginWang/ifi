#!/usr/bin/env python3
"""
Test suite for NasDB results cache functionality.

This module tests the new results directory checking functionality in NasDB,
...
@pytest.fixture
def nas_db():
    """Fixture to provide NasDB instance."""
...
def sample_results_h5(temp_results_dir):
    ...
    with h5py.File(h5_file, 'w') as f:
        ...
        signal_group = rawdata_group.create_group("sig_45821_056")
        signal_group.attrs["original_name"] = "45821_056.csv"
        signal_group.attrs["canonical_name"] = "45821_056.csv"
        ...
        for name, data in {
            ...
            dset = signal_group.create_dataset(name, data=data)
...
def multi_freq_results_h5(temp_results_dir):
    ...
    with h5py.File(h5_file, 'w') as f:
        ...
        freq_94_group = rawdata_group.create_group("sig_45821_056")
        freq_94_group.attrs["original_name"] = "45821_056.csv"
        freq_94_group.attrs["canonical_name"] = "45821_056.csv"
        freq_94_group.attrs["freq"] = 94.0e9
        freq_94_group.attrs["n_ch"] = 5
        freq_94_group.attrs["n_path"] = 2
        freq_94_group.attrs["meas_name"] = "94GHz Interferometer"
        for name, data in {
            "TIME": time_data_94,
            "CH0": signal_data_94,
            "CH1": signal_data_94 * 0.9,
        }.items():
            dset = freq_94_group.create_dataset(name, data=data)
            dset.attrs["original_name"] = name
        
        freq_280_group = rawdata_group.create_group("sig_45821_all")
        freq_280_group.attrs["original_name"] = "45821_ALL.csv"
        freq_280_group.attrs["canonical_name"] = "45821_ALL.csv"
        freq_280_group.attrs["freq"] = 282.0e9
        freq_280_group.attrs["n_ch"] = 1
        freq_280_group.attrs["n_path"] = 1
        freq_280_group.attrs["meas_name"] = "280GHz Interferometer"
        for name, data in {
            "TIME": time_data_280,
            "CH0": signal_data_280,
            "CH1": signal_data_280 * 0.8,
        }.items():
            dset = freq_280_group.create_dataset(name, data=data)
            dset.attrs["original_name"] = name
...
def invalid_results_h5(temp_results_dir):
    ...
    with h5py.File(h5_file, 'w') as f:
        ...
        f.attrs["ifi_version"] = "1.0"
        rawdata_group = f.create_group("rawdata")
        freq_group = rawdata_group.create_group("sig_45821_056")
        freq_group.attrs["original_name"] = "45821_056.csv"
        freq_group.attrs["canonical_name"] = "45821_056.csv"
        freq_group.attrs["freq"] = 94.0e9
        freq_group.attrs["n_ch"] = 5
        freq_group.attrs["n_path"] = 2
        freq_group.attrs["meas_name"] = "94GHz Interferometer"
        
        # Create data with NaN
        time_data = np.linspace(0, 0.01, 100)
        signal_data = np.full(100, np.nan)
        for name, data in {"TIME": time_data, "CH0": signal_data}.items():
            dset = freq_group.create_dataset(name, data=data)
            dset.attrs["original_name"] = name
...
class TestNasDB_ResultsCache:
    ...
    def test_load_signals_from_results_with_freq_filter(self, nas_db, multi_freq_results_h5):
        ...
        with nas_db:
            ...
            assert "45821_056.csv" in all_signals
            assert "45821_ALL.csv" in all_signals
            ...
            assert "45821_056.csv" in signals_94
            assert "45821_ALL.csv" not in signals_94
            ...
            assert "45821_ALL.csv" in signals_280
            assert "45821_056.csv" not in signals_280
