...
This module tests the basic functionality of NasDB and VestDB controllers,
...
@pytest.fixture
def vest_db():
    """Fixture to provide VestDB instance."""
    if not Path(CONFIG_PATH).exists():
        pytest.skip(f"Configuration file not found at '{CONFIG_PATH}'")
    return VestDB(config_path=CONFIG_PATH)


class TestNasDB:
    """Test suite for NasDB controller."""
    
    def test_initialization_and_connection(self, nas_db):
        """Test NasDB initialization and connection."""
        ...
        try:
            for attempt in range(max_retries):
                try:
                    ...
                    # Exit context manager to ensure NasDB connection is closed
...
class TestVestDB:
    """Test suite for VestDB controller."""
    
    def test_initialization_and_connection(self, vest_db):
        """Test VestDB initialization and connection."""
...
def run_all_tests():
    ...
    try:
        if Path(CONFIG_PATH).exists():
            # Test NasDB connection
            logger.info("Testing NasDB connection...")
            with NasDB(config_path=CONFIG_PATH) as nas:
                if nas._is_connected:
                    logger.info("  ✓ NasDB connection successful")
                else:
                    logger.warning("  ✗ NasDB connection failed")
            
            # Test VestDB connection
            logger.info("Testing VestDB connection...")
            with VestDB(config_path=CONFIG_PATH) as db:
                if db.connection and db.connection.open:
                    logger.info("  ✓ VestDB connection successful")
                else:
                    logger.warning("  ✗ VestDB connection failed")
        else:
