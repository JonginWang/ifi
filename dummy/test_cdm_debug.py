...
def debug_cdm_analysis():
    ...
    try:
        ...
        print(f"  - 신호 길이: {signal_length}, 최대 탭 수: {max_taps}")
        ...
        fig, ax = plotter.plot_density(density_df, title="CDM 밀도 분석 (디버깅)", show_plot=False)
        fig.savefig('cdm_debug_density.png', dpi=150, bbox_inches='tight')
        plt.close(fig)
        print("  - 디버깅 밀도 플롯 저장: cdm_debug_density.png")
        
        print("\n[SUCCESS] CDM 디버깅 완료!")
        
    except Exception as e:
        print(f"[ERROR] CDM 디버깅 실패: {e}")
        logger.error(f"CDM debugging failed: {e}")
        import traceback
        traceback.print_exc()
